...
/**
 * RentalFileService: the primary rental persistence service.
 * Uses the same rentals.txt as RentalService so all reads and writes
 * (bookings via PaymentController, admin approvals, dashboard stats)
 * operate on one consistent file.
 */
@Service
public class RentalFileService {
    ...
    /**
     * UPDATE Operation: Finds a rental by ID and changes its status, then rewrites the file.
     * @return true if the rental was found and updated, false otherwise.
     */
